# -*- coding: utf-8 -*-
# @Author        : Rui
# @Time          : 2024/4/24 14:31
# @Function      : DKN model for news recommendation system
import os
import numpy as np
import torch
...
from newsrec.utils import construct_entity_embedding


class DKNRSModel(BaseNRS):
    def __init__(self, **kwargs):
        ...
        self.user_layer = nn.Sequential(nn.Linear(news_embed_dim, 16), nn.Linear(16, 1))
        # self.news_layer, self.user_layer = None, None  # No attention for user layer and news layer

    def kcnn(self, news_embed, entity_embed=None):
        """
        ...
        if entity_embed is not None:
            ...
            news_vector = torch.stack([news_embed, entity_vector_trans], dim=1)
        else:
            # batch_size, word_embedding_dim, num_words_title
            news_vector = news_embed.transpose(1, 2)
        pooled_vectors = []
        for x in self.window_sizes:
    ...
    def user_attention(self, candidate_news_vector, clicked_news_vector):
        ...
        clicked_news_weights = torch.softmax(self.user_layer(concat_vector).squeeze(dim=2), dim=1)
    ...
    def news_encoder(self, input_feat):
        if self.news_entity_num:
            ...
            news_vector = self.dropout_ne(self.kcnn(word_vector))
        return {"news_vector": news_vector}
